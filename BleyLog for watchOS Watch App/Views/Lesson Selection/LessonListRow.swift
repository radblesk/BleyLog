...
struct LessonListRow: View {
    ...
    var body: some View {
        VStack(alignment: .leading) {
            ...
            Text(
                oneDay ? "Day \(lesson.firstDay)" : "Days \(lesson.firstDay) - \(lesson.lastDay)"
            )
            .font(.caption)
            .foregroundStyle(
                .secondary
            ...
            Text("^[\(lesson.projects.count) project](inflect: true)")
                .foregroundStyle(lesson.finished ? .green : lesson.inProgress ? .blue : .orange)
                .font(.footnote)
                .bold()
                .textCase(.uppercase)
        }
...
#Preview {
    List {
        LessonListRow(lesson: Lesson.exampleLesson)
    }
}
